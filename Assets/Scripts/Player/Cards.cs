    ...
    public class Cards
    {
        ...
        public void MonopolCardEffect(ResourceType chosenResource)
        {
            ...
            foreach (Player player in GameManager.State.Players)
            {
                if (player.index != GameManager.State.CurrentPlayerId)
                {
                    ...
                    GameManager.Logs.Add($"{player.name} mất {playerResourceNumber} {resourceName} loại " +
        ...
        private void UseKnightCard()
        {
                ...
                player.properties.cards.GetUsedKnightCardsNumber() >= usedKnightCards))
            {
                ...
                GameManager.Logs.Add($"Người chơi {GameManager.State.Players[GameManager.State.CurrentPlayerId].name} " +
                        $"đã sử dụng {GameManager.State.Players[GameManager.State.CurrentPlayerId].properties.cards.usedKnightCards} " +
                        $"thẻ Kị Sĩ và giành 2 điểm từ {playerWithRewardedKnights.name}");
            }
