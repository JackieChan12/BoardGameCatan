    ...
    public class BoardCreation : MonoBehaviour
    {
        ...
        private void SetupMapElements()
        {
            ...
            for (var i = 0; i < BoardManager.FieldsNumber; i++)
            {
                ...
                fields[i].transform.position = isMenu ? fieldPosition : new Vector3(fieldPositions[i, 0], -20f, fieldPositions[i, 1]);
                if (!isMenu)
                {
                    fields[i].GetComponent<FieldElement>().State.id = i;
                    var numComp = fields[i].GetComponent<NumberOverField.NumberOverField>();
                    if (numComp != null) numComp.HideNumber();
                }
            ...
            for (var i = 0; i < BoardManager.PortsNumber; i++)
            {
                ...
                ports[i].transform.position = isMenu ? portPosition : new Vector3(portPositions[i, 0], -20f, portPositions[i, 1]);
        ...
        private void SetBoardElementInfo()
        {
            ...
            if (!GameManager.LoadingGame)
            {
                //Destiny: Find desert field and place thief there; guard against missing desert (e.g. edge cases in advanced mode)
                var desertField = BoardManager.Fields.Where(field => field.type.Equals(FieldType.Desert))
                    .FirstOrDefault();
                if (desertField != null)
                {
                    desertField.SetThief(true);
                }
                else
                {
                    Debug.LogError("No Desert field found on the board. Thief could not be placed.");
                }
            }
        }
        ...
        private void SetPortInfo()
        {
            ...
            portInfos = new GameObject[portInfoPositions.Length / 2];
            for (var i = 0; i < portInfoPositions.Length / 2; i++)
            {
                portInfos[i] = Instantiate(portInfo);
                var targetY = portInfo.transform.position.y;
                portInfos[i].transform.position = 
                    new Vector3(portInfoPositions[i, 0], isMenu ? targetY : -20f, portInfoPositions[i, 1]);
                portInfos[i].GetComponent<NumberOverPort.NumberOverPort>().SetInfo(i);
                portInfos[i].SetActive(true);
            }            
        }
        ...
        void Start()
        {
            ...
            if (!isMenu)
            {
                ...
                // Start the board entry animation and UI deferral
                StartCoroutine(AnimateBoardStart());
            }

            GameManager.LoadingGame = false;
        }

        private System.Collections.IEnumerator AnimateBoardStart()
        {
            // 1. Find all Canvases and disable only UI (non-board) ones
            ...
            foreach (var canvas in canvases)
            {
                if (canvas != null)
                {
                    var isBoardCanvas = canvas.GetComponentInParent<FieldElement>() != null || 
                                        canvas.GetComponentInParent<JunctionElement>() != null;
                    if (!isBoardCanvas)
                    {
            ...
            try
            {
                // 2. Prepare slide-up elements (Fields, Ports and PortInfos)
                var slideElements = new System.Collections.Generic.List<SlideElement>();

                for (var i = 0; i < BoardManager.FieldsNumber; i++)
                {
                    if (fields[i] != null)
                    {
                        var targetPos = new Vector3(fieldPositions[i, 0], fieldLocationY, fieldPositions[i, 1]);
                        slideElements.Add(new SlideElement(fields[i], targetPos));
                    }
                }
                ...
                for (var i = 0; i < BoardManager.PortsNumber; i++)
                {
                    if (ports[i] != null)
                    {
                        var targetPos = new Vector3(portPositions[i, 0], portLocationY, portPositions[i, 1]);
                        slideElements.Add(new SlideElement(ports[i], targetPos));
                    }
                }
                ...
                if (portInfos != null)
                {
                    for (var i = 0; i < portInfos.Length; i++)
                    {
                        if (portInfos[i] != null)
                        {
                            var targetPos = new Vector3(portInfoPositions[i, 0], portInfo.transform.position.y+1, portInfoPositions[i, 1]);
                            slideElements.Add(new SlideElement(portInfos[i], targetPos));
                        }
                    }
                }

                // Sort by target Z coordinate ascending (from bottom of the screen to the top of the screen)
                slideElements.Sort((a, b) => a.TargetPosition.z.CompareTo(b.TargetPosition.z));

                // Start slide animations for each element with a small delay
                var activeAnimations = new System.Collections.Generic.List<Coroutine>();
                foreach (var elem in slideElements)
                {
                    if (elem.Obj != null)
                    {
                        activeAnimations.Add(StartCoroutine(SlideUp(elem.Obj, elem.TargetPosition)));
                        yield return new WaitForSeconds(0.06f); // delay between each element's start
                    }
                }
                ...
                yield return new WaitForSeconds(0.3f); // small pause before showing numbers

                // 3. Show numbers gradually in spiral shape from outside to inside
                // Spiral indices: 18, 15, 11, 6, 2, 1, 0, 3, 7, 12, 16, 17, 14, 10, 5, 4, 8, 13, 9
                int[] spiralIndices = { 18, 15, 11, 6, 2, 1, 0, 3, 7, 12, 16, 17, 14, 10, 5, 4, 8, 13, 9 };
                foreach (var index in spiralIndices)
                {
                    if (fields[index] != null)
                    {
                        var fieldState = (FieldState)fields[index].GetComponent<FieldElement>().State;
                        if (fieldState.number != 0)
                        {
                            var numComponent = fields[index].GetComponent<NumberOverField.NumberOverField>();
                            if (numComponent != null)
                            {
                                numComponent.ShowNumber();
            ...
            {
                // ALWAYS re-enable UI Canvases to show UI
        ...
        private System.Collections.IEnumerator SlideUp(GameObject obj, Vector3 targetPos)
        {
            float duration = 0.5f;
            float elapsed = 0f;
            Vector3 startPos = obj.transform.position;
            var interactiveElem = obj.GetComponent<Interactions.InteractiveElement>();
            ...
            while (elapsed < duration)
            {
                ...
                if (interactiveElem != null)
                {
                    interactiveElem.StartHeight = currentPos.y;
                }
                yield return null;
            }
            ...
            if (interactiveElem != null)
            {
                interactiveElem.StartHeight = targetPos.y;
            }
        }

        private struct SlideElement
        {
            public GameObject Obj;
            public Vector3 TargetPosition;

            public SlideElement(GameObject obj, Vector3 targetPos)
            {
                Obj = obj;
