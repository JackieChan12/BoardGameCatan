...
namespace Board
{
    public class ElementDistributor
    {
        ...
        {
            int playerId = GameManager.GetPlayerIdByColor(color);

            //Destiny: Guard against invalid player id (color not found among current players)
            if (playerId < 0 || playerId >= GameManager.State.Players.Length)
            {
                Debug.LogWarning($"SetupOnePlayerBuildings: player with color {color} not found. Skipping initial distribution.");
                return;
            }

            GameManager.State.Players[playerId].properties.AddBuilding(buildingId1, false, true);
            GameManager.State.Players[playerId].properties.AddBuilding(buildingId2, false, true);
            GameManager.State.Players[playerId].properties.AddPath(pathId1, true);
            GameManager.State.Players[playerId].properties.AddPath(pathId2, true);
        }
    }
}