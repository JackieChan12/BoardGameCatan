...
namespace Assets.Scripts.DataStorage.Managers
{
    ...
    {        
        ...
        public bool CheckIfCurrentPlayerCanUseCard(CardType cardType, int playerId = -1)
        {
            if (playerId == -1)
            {
            ...
            if (GameManager.State.Players == null || playerId < 0 || playerId >= GameManager.State.Players.Length || GameManager.State.Players[playerId] == null)
            {
                return false;
            }

            //Destiny: if player already used a card this round
            if (!GameManager.State.Players[playerId].canUseCard)
            {
                return false;
            }
            ...
            if (GameManager.State.Players[playerId].properties.cards.GetCardNumber(cardType) <= 0)
            {
                return false;
            }
        ...
        public bool CheckIfCurrentPlayerCanBuyCard(int playerId = -1)
        {
            if (playerId == -1)
            {
                playerId = GameManager.State.CurrentPlayerId;
            ...
            return !GameManager.PopupManager.CheckIfWindowShown() &&
                ...
                GameManager.State.Players[playerId].CanBuyCard();
        }
