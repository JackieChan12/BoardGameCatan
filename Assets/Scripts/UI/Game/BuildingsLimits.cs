...
namespace UI.Game
{
    public class BuildingsLimits : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            int activeId = playerId == -1 ? GameManager.State.CurrentPlayerId : playerId;
            if (activeId < 0 || activeId >= GameManager.State.Players.Length || GameManager.State.Players[activeId] == null)
            {
                return;
            }

            SetLabelState(LabelType.VILLAGES, GameManager.BuildManager.CheckIfPlayerCanBuildAnyBuilding(JunctionType.Village, activeId));
            SetLabelState(LabelType.CITES, GameManager.BuildManager.CheckIfPlayerCanBuildAnyBuilding(JunctionType.City, activeId));
            SetLabelState(LabelType.PATHS, GameManager.BuildManager.CheckIfPlayerCanBuildAnyPath(activeId));
            SetLabelState(LabelType.CARDS, GameManager.CardsManager.CheckIfCurrentPlayerCanBuyCard(activeId));

            var villages = GameManager.State.Players[activeId].properties.GetVillagesNumber();
            var cities = GameManager.State.Players[activeId].properties.GetCitiesNumber();
            var paths = GameManager.State.Players[activeId].properties.GetPathsNumber();
            var cards = GameManager.State.Players[activeId].properties.cards.GetCardNumber();
