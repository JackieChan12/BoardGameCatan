    ...
    public class ThiefPlayerChoicePopupController : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            for (var i = 0; i < GameManager.State.Players.Length; i++)
            {
                ...
                playersResourcesNumber[i].text = $"Tài nguyên sở hữu: {GameManager.State.Players[i].resources.GetResourceNumber()}";
            }
        ...
        private void OnConfirmButton()
        {
            ...
            GameManager.Logs.Add($"{GameManager.State.Players[chosenPlayerIndex].name} đưa 1 đơn vị tài nguyên {resourceName} "
                + $"cho người chơi {GameManager.State.Players[GameManager.State.CurrentPlayerId].name}");
