...
namespace UI.Game
{
    ...
    public class ActionsContentNavigation : MonoBehaviour
    {
        ...
        private void OnBuyCardButton()
        {
            ...
            GameManager.PopupManager.LastBoughtCard = GameManager.State.Players[playerId].BuyCard();
        }
        ...
        private void OnCancelButton()
        {
            ...
            {
                GameManager.State.Players[playerId].CancelBuilding(element.Item2);
            }
            else if (element.Item1 == ElementType.Path)
            {
                GameManager.State.Players[playerId].CancelPath(element.Item2);
            }

            if (GameManager.BuildManager.BuildingHistory.Count == 0)
            {
                bool playerBoughtCard = GameManager.State.Players[playerId]
        ...
        private void OnTurnSkipButton()
        {
            ...
            GameManager.State.Players[playerId].canUseCard = true;
        ...
        private void InvokeBuildProcedure()
        {
            ...
            if (GameManager.Selected.Element as JunctionElement != null)
            {
                ...
                GameManager.State.Players[playerId].BuildBuilding(element);
            }
            else if (GameManager.Selected.Element as PathElement != null)
            {
                var element = (PathElement)GameManager.Selected.Element;
                GameManager.State.Players[playerId].BuildPath(element);
            }
        }
        ...
        private void TradeButtonActivity()
        {
            ...
            if (landTradeButton != null) landTradeButton.interactable = val;
            if (seaTradeButton != null) seaTradeButton.interactable = val;
        }
        ...
        private void TurnSkipButtonActivity()
        {
            ...
            if (GameManager.State.SwitchingGameMode == SwitchingMode.InitialSwitchingFirst)
            {
                var val =  GameManager.State.Players[playerId].properties.GetBuildingsNumber() == 1 &&
                           GameManager.State.Players[playerId].properties.GetPathsNumber() == 1;
            ...
            {
                var val =  GameManager.State.Players[playerId].properties.GetBuildingsNumber() == 2 &&
        ...
        {
            cardsLeftInfo.text = "Số thẻ còn lại: " +
                                 $"{GameManager.CardsManager.Deck.Count}";
            
            // Chỉ người chơi đang trong lượt mới được xử lý các yêu cầu xây dựng/di chuyển thief từ hàng đợi dùng chung
        ...
        {
            if (turnSkipButton != null) turnSkipButton.interactable = false;
            if (turnSkipIcon != null) turnSkipIcon.color = Color.gray;
            
            if (buyCardButton != null) buyCardButton.interactable = false;
            if (buyCardIcon != null) buyCardIcon.color = Color.gray;
            
            if (tradeButton != null) tradeButton.interactable = false;
            if (tradeIcon != null) tradeIcon.color = Color.gray;
            
            if (cancelButton != null) cancelButton.interactable = false;
            if (cancelIcon != null) cancelIcon.color = Color.gray;
